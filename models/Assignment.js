...
class Assignment {
    ...
    async create(data) {
        try {
            ...
            // Check if teacher exists (teacherId refers to Teacher.id, not User.id)
            ...
            if (!teacherRecord) {
                throw new Error(`Teacher with ID ${data.teacherId} not found`);
            }

            if (!teacherRecord.user) {
                throw new Error(`Teacher with ID ${data.teacherId} does not have an associated User record`);
            }

            // Verify teacher belongs to the same school
            if (teacherRecord.user.schoolId !== data.schoolId) {
                throw new Error('Teacher does not belong to the specified school');
            }

            // Update the teacherId to be the User.id for the assignment creation
            data.teacherId = teacherRecord.user.id;
            ...
            if (data.subjectId) {
                ...
                if (!subjectExists) {
                    throw new Error('Subject not found or does not belong to the specified school');
                }
            }
    ...
    async getById(id, userId, schoolId, userRole) {
        try {
            // Validate and convert IDs - handle both string and BigInt inputs
            let assignmentId, schoolIdNum;
            ...
            // Handle school ID
            if (typeof schoolId === 'bigint') {
                schoolIdNum = Number(schoolId);
            } else {
                schoolIdNum = parseInt(schoolId);
            }
            
            // Debug logging
            console.log('Assignment.getById - Debug:', {
                id: id,
                schoolId: schoolId,
                assignmentId: assignmentId,
                schoolIdNum: schoolIdNum,
                isNaN_assignmentId: isNaN(assignmentId),
                isNaN_schoolIdNum: isNaN(schoolIdNum),
                idType: typeof id,
                schoolIdType: typeof schoolId
            });
            
            if (isNaN(assignmentId) || isNaN(schoolIdNum)) {
                throw new Error(`Invalid assignment ID or school ID - assignmentId: ${id} (${assignmentId}), schoolId: ${schoolId} (${schoolIdNum})`);
            }
            
            let where = {
                id: BigInt(assignmentId),
                schoolId: BigInt(schoolIdNum),
                deletedAt: null
            };
            ...
            if (userRole === 'STUDENT') {
                ...
                where.teacherId = typeof userId === 'bigint' ? userId : BigInt(userId);
            }
    ...
    async getAll(filters = {}, pagination = {}) {
        try {
            ...
            // Debug logging
            console.log('🔍 Assignment.getAll called with filters:', {
            ...
            console.log('📊 Assignment count result:', total);
    ...
    async update(id, data, userId, schoolId) {
        try {
            ...
            if (isNaN(assignmentId) || isNaN(schoolIdNum) || isNaN(userIdNum)) {
                throw new Error('Invalid assignment ID, school ID, or user ID');
            }
