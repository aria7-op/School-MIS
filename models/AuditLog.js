        ...
        try {
            const auditLog = await this.prisma.auditLog.create({
                data: {
                    ...
                    entityId: data.entityId !== undefined && data.entityId !== null ? BigInt(data.entityId) : BigInt(0),
    ...
    async getAll(filters = {}) {
        try {
            ...
            if (typeof isSuccess === 'boolean') {
                ...
                if (isSuccess.toLowerCase() === 'false') where.isSuccess = false;
            }
            if (ipAddress) where.ipAddress = ipAddress;
            if (requestPath) where.requestPath = { contains: requestPath, mode: 'insensitive' };
            if (requestUrl) where.requestUrl = { contains: requestUrl, mode: 'insensitive' };
            ...
            const [logs, total] = await Promise.all([
                this.prisma.auditLog.findMany({
                    ...
                    include: {
                        ...
                        user: { select: { id: true, firstName: true, lastName: true, displayName: true, username: true } },
