...
try {
  notificationService = require('../services/notifications/notificationService').default;
} catch (error) {
  console.warn('Notification service not available, using fallback:', error);
...
export const NotificationProvider: React.FC<NotificationProviderProps> = ({ 
  ...
  const requestPushPermissions = async (): Promise<boolean> => {
    try {
      // Skip on web or if messaging is not available
      ...
      if (Platform.OS === 'ios') {
        ...
        if (!enabled) {
          Alert.alert(
            'Push Notifications',
            'Please enable push notifications in Settings to receive important updates.',
            [
        ...
        if (!granted) {
          Alert.alert(
            'Push Notifications',
            'Please enable push notifications to receive important updates.',
            [
              { text: 'Cancel', style: 'cancel' },
              { text: 'Settings', onPress: () => {} }
            ]
    ...
    try {
      ...
      if (response && response.unread !== undefined) {
        ...
        if (Platform.OS === 'ios' || Platform.OS === 'android') {
          try {
            // @ts-ignore
            const { default: RNBadge } = require('react-native-badge');
