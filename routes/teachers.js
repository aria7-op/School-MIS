...
import { enforcePackageLimit } from '../middleware/packageLimits.js';
import { countTeachersForSchool } from '../services/subscriptionService.js';

const router = express.Router();
const teacherCreationLimit = enforcePackageLimit({
  ...
  counter: async (req) => {
    ...
    const incoming = Array.isArray(req.body?.teachers) ? req.body.teachers.length : 1;
    return current + incoming;
  },
  message: 'Teacher limit reached for your current subscription. Please upgrade your plan to add more teachers.',
});
...
router.post('/',
  authenticateToken,
  authorizeRoles(['SUPER_DUPER_ADMIN', 'SUPER_ADMIN', 'SCHOOL_ADMIN', 'TEACHER']),
  authorizePermissions(['teacher:create']),
  teacherCreateLimiter,
  validateBody(TeacherCreateSchema),
  teacherCreationLimit,
...
router.get('/cache/stats',
  authenticateToken,
  authorizeRoles(['SUPER_DUPER_ADMIN', 'SUPER_ADMIN']),
...
/**
 * @route   POST /api/teachers/create-from-users
 * @desc    Create teacher records for existing SCHOOL_ADMIN users
 * @access  Private (SUPER_ADMIN, SCHOOL_ADMIN, TEACHER)
 * @permissions teacher:create
 */
router.post('/create-from-users',
  authenticateToken,
  authorizeRoles(['SUPER_DUPER_ADMIN', 'SUPER_ADMIN', 'SCHOOL_ADMIN', 'TEACHER']),
  authorizePermissions(['teacher:create']),
  async (req, res) => {
    try {
      ...
      const schoolAdminUsers = await require('../generated/prisma/index.js').PrismaClient().user.findMany({
        where: {
          role: 'SCHOOL_ADMIN',
          deletedAt: null,
          teacher: null,
      ...
      const results = [];
      const prisma = new (require('../generated/prisma/index.js').PrismaClient)();
        ...
        try {
          ...
          const teacher = await prisma.teacher.create({
            data: {
              ...
              schoolId: user.schoolId || BigInt(schoolId),
          ...
          results.push({
            success: true,
            teacher: teacher,
            message: `Created teacher record for ${user.firstName} ${user.lastName}`
          });
        } catch (error) {
          results.push({
            success: false,
      ...
      res.json({
        success: true,
        message: `Created ${results.filter(r => r.success).length} teacher records`,
        data: results
      });
    } catch (error) {
      res.status(500).json({
