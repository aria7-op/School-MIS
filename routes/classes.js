...
/**
 * @route   POST /api/classes/assign-teachers
 * @desc    Assign multiple teachers to a class
 * @access  Private (SUPER_ADMIN, SCHOOL_ADMIN, TEACHER)
 * @body    {classId: number, teacherIds: number[]}
...
router.post('/assign-teachers',
  ...
  validateRequest(z.object({
    classId: idSchema.shape.id,
    teacherIds: z.array(idSchema.shape.id)
  }), 'body'),
  classController.assignMultipleTeachersToClass
);

/**
 * @route   GET /api/classes/:classId/teachers
 * @desc    Get teachers assigned to a class
 * @access  Private (SUPER_ADMIN, SCHOOL_ADMIN, TEACHER)
 * @params  {classId} - Class ID
 * @permissions class:read
 */
router.get('/:classId/teachers',
  authenticateToken,
  authorizeRoles(['SUPER_ADMIN', 'SCHOOL_ADMIN', 'TEACHER']),
  authorizePermissions(['class:read']),
  validateRequest(z.object({ classId: idSchema.shape.id }), 'params'),
  classController.getClassTeachers
);

/**
 * @route   PUT /api/classes/:classId/supervisor
 * @desc    Set or update class supervisor (نگران صنف)
 * @access  Private (SUPER_ADMIN, SCHOOL_ADMIN, TEACHER)
 * @params  {classId} - Class ID
 * @body    {supervisorId: number | null}
 * @permissions class:update
 */
router.put('/:classId/supervisor',
  authenticateToken,
  authorizeRoles(['SUPER_ADMIN', 'SCHOOL_ADMIN', 'TEACHER']),
  authorizePermissions(['class:update']),
  validateRequest(z.object({ classId: idSchema.shape.id }), 'params'),
  auditLog('UPDATE', 'Class'),
  classController.setClassSupervisor
);
 ...
 * @desc    Get classes for the current authenticated teacher
 ...
 * @desc    Link a subject to a class
 * @access  Private (SUPER_ADMIN, SCHOOL_ADMIN)
 * @params  {id} - Class ID
 * @body    {subjectId}
 * @permissions class:update, subject:read
 */
router.post('/:id/subjects',
  authenticateToken,
  authorizeRoles(['SUPER_ADMIN', 'SCHOOL_ADMIN']),
  authorizePermissions(['class:update', 'subject:read']),
  validateRequest(z.object({ id: idSchema.shape.id }), 'params'),
  validateRequest(z.object({ subjectId: z.number().int().positive() }), 'body'),
  classController.addSubjectToClass
);
 ...
 * @access  Private (SUPER_ADMIN, SCHOOL_ADMIN)
 * @params  {classId} - Class ID, {teacherId} - Teacher ID
 * @permissions class:update
 */
