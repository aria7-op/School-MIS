...
const computeFilesSize = (req) => {
    const files = Array.isArray(req.files) ? req.files : [];
    return files.reduce((total, file) => total + (Number(file.size) || 0), 0);
};

const hostelImageStorageLimit = enforceStorageLimit({
    byteCounter: (req) => computeFilesSize(req),
    message: 'Hostel media upload exceeds your subscription storage allowance. Please upgrade or remove existing files.',
});
