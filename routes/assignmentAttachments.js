...
import { rateLimiter, fileUploadLimiter } from '../middleware/rateLimit.js';
import { enforceStorageLimit } from '../middleware/packageLimits.js';
import multer from 'multer';
import path from 'path';
import fs from 'fs-extra';
import { v4 as uuidv4 } from 'uuid';
import { sanitizeFilename } from '../middleware/fileSecurity.js';
...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        const targetDir = path.resolve('uploads/assignments');
    ...
    filename: (req, file, cb) => {
        ...
        cb(null, `${safeBaseName || file.fieldname}-${uniqueSuffix}${ext}`);
    }
});
...
const computeFileBytes = (req) => {
    const file = req.file;
    if (!file) return 0;
    return file.size || 0;
};

const enforceAttachmentStorageLimit = enforceStorageLimit({
    byteCounter: (req) => computeFileBytes(req),
});
