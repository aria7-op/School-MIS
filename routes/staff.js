...
import { countStaffForSchool } from '../services/subscriptionService.js';

const router = express.Router();
const staffCreationLimit = enforcePackageLimit({
  limitKey: 'maxStaff',
  counter: async (req) => {
    const schoolId = req.body?.schoolId || req.user?.schoolId;
    if (!schoolId) return 0;
    const current = await countStaffForSchool(schoolId);
    const incomingArray = Array.isArray(req.body?.staff)
      ? req.body.staff
      : Array.isArray(req.body?.staffMembers)
        ? req.body.staffMembers
        : Array.isArray(req.body?.records)
          ? req.body.records
          : null;
    const incoming = incomingArray ? incomingArray.length : 1;
    return current + incoming;
  },
  message: 'Staff limit reached for your current subscription. Please upgrade your plan to add more staff members.',
});

const staffDocumentStorageLimit = enforceStorageLimit({
  byteCounter: (req) => Number(req.file?.size || 0),
  message:
    'Staff document upload exceeds the storage allowance for your subscription. Please upgrade your plan or remove existing files.',
});
...
router.post('/import',
  authenticateToken,
  authorizeRoles(['SUPER_DUPER_ADMIN']),
