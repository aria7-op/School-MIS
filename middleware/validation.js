...
export const validateSchoolAccess = async (user, schoolId) => {
  try {
    ...
    // Super admins have access to their assigned school
    if (user.role === 'SUPER_ADMIN') {
      console.log('SUPER_ADMIN detected, checking school access...');
      console.log('User schoolId:', user.schoolId, 'Requested schoolId:', schoolId);
      ...
      // SUPER_ADMIN can access their assigned school
      if (user.schoolId && BigInt(user.schoolId) === BigInt(schoolId)) {
        console.log('School access validated for SUPER_ADMIN - user is assigned to this school');
        return;
      }
      
      console.log('School does not match SUPER_ADMIN user assignment');
...
export const validateClassAccess = async (user, classId, schoolId) => {
  try {
    ...
    const classIdBigInt = typeof classId === 'bigint' ? classId : BigInt(classId);
    const schoolIdBigInt = schoolId === null || schoolId === undefined
      ? null
      : typeof schoolId === 'bigint'
        ? schoolId
        : BigInt(schoolId);

    const baseClassWhere = {
      id: classIdBigInt,
      isActive: true,
      deletedAt: null
    };

    if (schoolIdBigInt !== null) {
      baseClassWhere.schoolId = schoolIdBigInt;
    }

    // Check if class exists, belongs to the school (when applicable), and is active
    let classData = await prisma.class.findFirst({
      where: baseClassWhere,
      select: {
        id: true,
        schoolId: true,
        name: true,
    ...
    // For super admins, allow access if the class exists regardless of school match
    if (!classData && user.role === 'SUPER_ADMIN') {
    ...
    if (!classData) {
      const error = new Error('Class not found, does not belong to this school, or is inactive');
      error.statusCode = 404;
      throw error;
    }
...
export const sanitizeString = (str, { allowHtml = false } = {}) => {
  ...
  // validator.escape will encode &, <, >, ", and '
 ...
 */
export const sanitizeObject = (obj, path = [], seen = new WeakSet()) => {
  ...
  if (typeof obj === 'string') {
    const allowHtml = !!lastKey && RICH_TEXT_FIELDS.has(lastKey);
  ...
  if (Array.isArray(obj)) {
    return obj.map((item, index) => sanitizeObject(item, [...path, String(index)], seen));
  }

  const sanitized = {};
  for (const [key, value] of Object.entries(obj)) {
    sanitized[key] = sanitizeObject(value, [...path, key], seen);
  }
  return sanitized;
};
