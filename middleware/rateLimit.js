...
const buildStore = (windowMs, name) =>
...
const isExemptPath = (req) => {
  ...
  return RATE_LIMIT_EXEMPT_PATHS.has(url);
};
...
const generalLimiter = rateLimit({
  ...
  store: buildStore(FIFTEEN_MINUTES, 'general'),
  keyGenerator: (req) => {
    // Use user ID if authenticated, otherwise use IP (IPv6 safe)
    return req.user ? `user:${req.user.id}` : req.ip;
  },
  skip: (req) => {
    // Skip rate limiting for health checks or exempt paths
    if (req.path === '/health' || req.path === '/api/health') {
      return true;
    }
    return isExemptPath(req);
  }
});
...
const resolvePackageRateLimit = (req) => {
  ...
  const candidate =
    ...
    features.api_requests_per_min ??
    features.apiRateLimitPerMin ??
    features.apiRateLimitPerMinute ??
    features.api_requestsPerMinute ??
    features.apiRateLimit;
...
const packageAwareLimiter = ({ windowMs = 60 * 1000, defaultMax = 200, floor = 25 } = {}) =>
  rateLimit({
    windowMs,
    max: (req, res) => {
      const resolved = resolvePackageRateLimit(req);
      ...
      return Math.max(Math.floor(numeric), floor);
    },
    message: {
      success: false,
      error: 'Too many requests',
...
const authLimiter = rateLimit({
  ...
  handler: async (req, res) => {
    ...
    securityMonitor.recordEvent('auth:rate_limit', {
      ip: req.ip,
...
const fileUploadLimiter = rateLimit({
  ...
  handler: async (req, res) => {
    ...
    securityMonitor.recordEvent('upload:rate_limit', {
      userId: req.user?.id,
      ip: req.ip,
    });
