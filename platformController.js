...
class PlatformController {
  ...
  async getFinancialAnalytics(req, res) {
    try {
      const params = z.object({ range: z.enum(['7d', '30d', '90d', '1y']).optional() }).parse(req.query);
      const rangeKey = params.range || '30d';
      const cacheKey = `financial:${rangeKey}`;
      ...
      const revenue = paidPayments.reduce((acc, payment) => acc + Number(payment.total || 0), 0);

      const revenueTrend = payments.reduce((acc, payment) => {
        if (!payment.paymentDate) return acc;
        const day = payment.paymentDate.toISOString().slice(0, 10);
        acc[day] = (acc[day] || 0) + Number(payment.total || 0);
        return acc;
      }, {});

      const monthlyRevenue = paidPayments.reduce((acc, payment) => {
        if (!payment.paymentDate) return acc;
        const month = payment.paymentDate.toISOString().slice(0, 7); // YYYY-MM
        acc[month] = (acc[month] || 0) + Number(payment.total || 0);
        return acc;
      }, {});
      ...
      const mrr = days > 0 ? (revenue / days) * 30 : revenue;
      const arr = mrr * 12;
      const averageRevenuePerSchool = uniquePayingSchools ? revenue / uniquePayingSchools : 0;
      const averageTransactionValue = paidPayments.length ? revenue / paidPayments.length : 0;
      ...
      const payload = {
        ...
        data: {
          ...
          averageRevenuePerSchool: Number(averageRevenuePerSchool.toFixed(2)),
