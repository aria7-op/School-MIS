...
import {
  sanitizeFilename,
  secureFileDownload,
  secureStaticFileServing,
} from "./middleware/fileSecurity.js";
...
const createDatabaseSslConfig = () => {
  ...
  try {
    ...
    throw new Error(
      `Failed to read DB SSL CA certificate from ${caPath}: ${error.message}`
    );
  }
};
...
io.use(async (socket, next) => {
  try {
    ...
    if (!resolvedToken) {
      logger.warn("websocket:no-token", {
        socketId: socket.id,
        headers: maskHeaders(socket.handshake?.headers),
      });
...
io.on("connection", (socket) => {
  ...
  socket.rateLimitKey = socket.userId
    ? `user:${socket.userId}`
    : `socket:${socket.id}`;
  ...
  socket.use(async (packet, next) => {
    ...
    if (validator) {
      try {
        ...
        socket.emit("error", {
          message: "Invalid payload for realtime action.",
        });
  ...
  socket.on("notification:read", async (data) => {
    try {
      ...
      logger.error("websocket:notification-read-error", error, {
        userId: socket.userId,
      });
    }
  });
  ...
  socket.on("notification:delete", async (data) => {
    try {
      ...
      logger.error("websocket:notification-delete-error", error, {
        userId: socket.userId,
      });
    }
  });
  ...
  try {
    ...
    if (!dbConfig) {
      ...
      if (dbUser && dbUser.toLowerCase() === "root") {
        throw new Error(
          'DB_USER must not be "root". Configure a least-privilege database account.'
        );
      }
      ...
      throw new Error(
        "DATABASE_URL must not contain root user credentials. Configure a least-privilege database account."
      );
    }
    ...
    if (
      ...
      try {
        const altDbConfig = parseDatabaseUrl(
        ...
        if (altDbConfig) {
          ...
          try {
            ...
            logger.warn("database:utf8-enforce-alt-failed", {
              message: e?.message,
            });
          }
...
app.use(
  requestAuditMiddleware({
    ignorePatterns: [/^\/api\/status/i, /^\/api\/database\/status/i],
  })
);
...
const mergeAllowedHeaders = (req) => {
  ...
  const combined = Array.from(
    new Set(DEFAULT_ALLOWED_HEADERS_ARRAY.concat(requested.filter(Boolean)))
  );
...
const resolveMaxStorageGb = (req) => {
  ...
  const featuresRaw =
    req.subscriptionFeatures || req.subscription?.package?.features || {};
  ...
  if (typeof featuresRaw === "string") {
    try {
      ...
      logger.warn("subscription:package-features-parse-error", {
  ...
  try {
    ...
    try {
      ...
      return res.status(400).json({
        ...
        message:
          "Uploaded file failed security screening. Please upload a different file.",
      });
    }
  ...
  try {
    ...
    try {
      ...
      return res.json({
        success: true,
        message:
          "Converted students available (mock data due to database issue)",
        data: {
          students: [
            {
              id: 1,
...
app.use((err, req, res, next) => {
  if (
    ...
    err?.code === "UNSUPPORTED_FILE_EXTENSION"
  ) {
  ...
  try {
    ...
    server.on("error", (error) => {
      ...
      if (error.code === "EADDRINUSE") {
        logger.error("server:port-in-use", new Error("Port already in use"), {
          port: PORT,
        });
      }
    });
