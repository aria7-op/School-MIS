...
export const getAllCustomers = async (req, res) => {
  try {
    ...
    resolveScopeOrReject(scope, 'customer list');
    ...
    if (minValue || maxValue) {
      ...
      if (minValue) baseWhere.totalSpent.gte = parseFloat(minValue);
      if (maxValue) baseWhere.totalSpent.lte = parseFloat(maxValue);
    }
    ...
    const { where } = ensureScopedCustomerWhere(scope, baseWhere);
    ...
    return respondWithScopedError(res, error, 'Failed to retrieve customers');
  }
};
...
export const getCustomerById = async (req, res) => {
  try {
    ...
    resolveScopeOrReject(scope, 'customer detail');
    ...
    const customer = await prisma.customer.findFirst({
      where: ensureScopedCustomerWhere(scope, { id: customerId }).where,
      include: includeClause
    });
    ...
    return respondWithScopedError(res, error, 'Failed to retrieve customer');
  }
};
...
export const createCustomer = async (req, res) => {
    ...
    try {
      ...
      resolveScopeOrReject(scope, 'customer create');

      let customerData = req.body;
      const createdBy = toBigIntSafe(req.user?.id);
      ...
      const validFields = [
        ...
        'schoolId', 'ownerId', 'pipelineStageId', 'createdAt', 'updatedAt', 'deletedAt', 'userId', 'branchId'
      ];
      ...
      const requestedBranchId = toBigIntOrNull(filteredCustomerData.branchId);
      if (scope.branchId && requestedBranchId && scope.branchId !== requestedBranchId) {
        return res.status(403).json({ success: false, message: 'Branch does not match selected context' });
      }
      filteredCustomerData.branchId = scope.branchId ?? requestedBranchId ?? null;

      if (filteredCustomerData.pipelineStageId) {
        const stageAccessible = await ensurePipelineStageInScope(filteredCustomerData.pipelineStageId, scope);
        if (!stageAccessible) {
          return res.status(404).json({ success: false, message: 'Pipeline stage not found in the selected context' });
        }
        filteredCustomerData.pipelineStageId = toBigIntSafe(filteredCustomerData.pipelineStageId);
      }

      if (filteredCustomerData.ownerId) {
        const ownerAccessible = await ensureOwnerInScope(filteredCustomerData.ownerId, scope);
        if (!ownerAccessible) {
          return res.status(404).json({ success: false, message: 'Owner not found in the selected context' });
        }
        filteredCustomerData.ownerId = toBigIntSafe(filteredCustomerData.ownerId);
      }

      filteredCustomerData.schoolId = toBigIntSafe(scope.schoolId);
      ...
      return respondWithScopedError(res, error, 'Failed to create customer');
    }
};
...
export const updateCustomer = async (req, res) => {
  try {
    ...
    resolveScopeOrReject(scope, 'customer update');
    ...
    const updatedBy = toBigIntSafe(req.user?.id);
    ...
    return respondWithScopedError(res, error, 'Failed to update customer');
  }
};

export const partialUpdateCustomer = async (req, res) => {
  try {
    const scope = await resolveCustomerScope(req, 'customer partial update');
    const { id } = req.params;
    if (!/^[0-9]+$/.test(id)) {
      return res.status(400).json({ success: false, message: 'Invalid customer ID' });
    }
    ...
    return respondWithScopedError(res, error, 'Failed to partially update customer');
  }
};

export const deleteCustomer = async (req, res) => {
  try {
    const scope = await resolveCustomerScope(req, 'customer delete');
    const { id } = req.params;
    if (!/^[0-9]+$/.test(id)) {
      return res.status(400).json({ success: false, message: 'Invalid customer ID' });
    }
    ...
    const deletedBy = toBigIntSafe(req.user?.id);
    ...
    return respondWithScopedError(res, error, 'Failed to delete customer');
  }
};
...
export const getCustomerDashboard = async (req, res) => {
  try {
    const scope = await resolveCustomerScope(req, 'customer dashboard');
    const { id } = req.params;
    if (!/^[0-9]+$/.test(id)) {
      return res.status(400).json({ success: false, message: 'Invalid customer ID' });
    }
    ...
    res.json({
      ...
      meta: { customerId: Number(customer.id) }
    });

  } catch (error) {
    return respondWithScopedError(res, error, 'Failed to get customer dashboard');
  }
};

export const getCustomerReports = async (req, res) => {
  try {
    const scope = await resolveCustomerScope(req, 'customer reports');
    const { 
      reportType = 'summary',
      dateFrom, 
      dateTo, 
    ...
    return respondWithScopedError(res, error, 'Failed to generate customer report');
  }
};

export const getCustomerComparisons = async (req, res) => {
  try {
    const scope = await resolveCustomerScope(req, 'customer comparisons');
    ...
    const ids = customerIds.split(',').map(toBigIntOrNull);
    if (ids.some(id => !id)) {
      return res.status(400).json({ success: false, message: 'Invalid customer ID provided' });
    }

    for (const id of ids) {
      const accessible = await ensureCustomerInScope(id, scope);
      if (!accessible) {
        return res.status(404).json({ success: false, message: 'One or more customers not found in the selected context' });
      }
    }

    const customers = await prisma.customer.findMany({
      where: ensureScopedCustomerWhere(scope, { id: { in: ids } }).where,
      select: {
        id: true,
        firstName: true,
        lastName: true,
    ...
    res.json({
      ...
      meta: { 
        customerIds: ids.map(id => Number(id)),
        metrics,
    ...
    return respondWithScopedError(res, error, 'Failed to compare customers');
  }
};
  ...
  try {
    ...
    if (schoolId) {
      try {
        ...
        console.warn('Failed to refresh subscription usage after customer document upload:', usageError);
      }
    }
...
const ensureScopedCustomerWhere = (scope, baseWhere = {}) => {
  return { where: applyScopeToWhere({ ...baseWhere }, scope, { useCourse: false }) };
};
...
const resolveCustomerScope = async (req, entityName) => {
  const scope = normalizeScopeWithSchool(
    await resolveManagedScope(req),
    toBigIntSafe(req.user?.schoolId)
  );
  return resolveScopeOrReject(scope, entityName);
};