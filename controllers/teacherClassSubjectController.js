...
class TeacherClassSubjectController {
  ...
  async getAssignments(req, res) {
    try {
      ...
      if (result.pagination) {
        responsePayload.pagination = convertBigIntToString(result.pagination);
      }
  ...
  async bulkAssign(req, res) {
    try {
      const { assignments, supervisorId, classId } = req.body;
      const schoolId = req.user.schoolId;
      const assignedBy = req.user.id;
      ...
      // If supervisorId is provided, update the class supervisor
      if (supervisorId && classId) {
        try {
