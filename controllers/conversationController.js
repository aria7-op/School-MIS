...
import { logger as securityLogger } from '../utils/logger.js';
...
class AdvancedConversationController {
  ...
  async sendMessage(req, res) {
    try {
      ...
      // Log message event
      securityLogger.messageEvent('send', 'success', {
      ...
      // Track message activity for burst detection
      securityMonitor.recordEvent('message:send', {
        userId: req.user.id,
        ip: req.ip,
        conversationId: id,
      });
  ...
  /**
   * WebSocket token generation removed - no longer needed
   */
  async generateWebSocketToken(req, res) {
    return createErrorResponse(res, 410, 'WebSocket functionality has been removed. Use polling-based messaging instead.');
  }
