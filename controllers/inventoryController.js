...
const { validateInventoryItemData, validateCategoryData, validateSupplierData, validatePurchaseOrderData } = require('../utils/inventoryUtils');
const { generateSKU, generateBarcode, calculateInventoryValue } = require('../utils/inventoryUtils');
const { cacheInventoryItem, invalidateInventoryCache } = require('../cache/inventoryCache');
const { searchInventoryItems, getInventoryAnalytics } = require('../services/inventoryService');
...
class InventoryController {
  ...
  async createCategory(req, res) {
    try {
      ...
      if (!categoryData.code) {
        categoryData.code = await generateSKU('CAT', schoolId);
      }
  ...
  async createItem(req, res) {
    try {
      const { error, value } = validateInventoryItemData(req.body);
      if (error) {
        return res.status(400).json({ success: false, message: error.details[0].message });
      }
      ...
      if (!itemData.sku) {
        itemData.sku = await generateSKU('INV', schoolId);
      }
      if (!itemData.barcode) {
        itemData.barcode = await generateBarcode(schoolId);
      }
  ...
  async updateItem(req, res) {
    try {
      ...
      const { error } = validateInventoryItemData(updateData, true);
      if (error) {
        return res.status(400).json({ success: false, message: error.details[0].message });
      }
  ...
  async createSupplier(req, res) {
    try {
      const { error, value } = validateSupplierData(req.body);
      if (error) {
        return res.status(400).json({ success: false, message: error.details[0].message });
      }
      ...
      if (!supplierData.code) {
        supplierData.code = await generateSKU('SUP', schoolId);
      }
  ...
  async createPurchaseOrder(req, res) {
    try {
      const { error, value } = validatePurchaseOrderData(req.body);
      if (error) {
        return res.status(400).json({ success: false, message: error.details[0].message });
      }
      ...
      // Generate PO number
      const poNumber = await generateSKU('PO', schoolId);
  ...
  async searchItems(req, res) {
    try {
      ...
      const searchResults = await searchInventoryItems(schoolId, {
        query,
        categoryId,
        supplierId,
        status,
  ...
  async getInventoryAnalytics(req, res) {
    try {
      ...
      const analytics = await getInventoryAnalytics(schoolId, {
        startDate,
        endDate
      });
  ...
  async generateInventoryReport(req, res) {
    try {
      ...
      const totalValue = await calculateInventoryValue(schoolId);
