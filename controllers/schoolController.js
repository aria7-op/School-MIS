...
class SchoolController {
  ...
  async createSchool(req, res) {
    try {
      ...
      // Subscription limit checks (non platform-admin users)
      if (!user?.isSuperDuperAdmin) {
        const limits = req.subscriptionLimits || {};
        if (limits?.maxSchools !== null && limits?.maxSchools !== undefined) {
          const ownerId = body.ownerId || req.schoolOwnerId || user?.createdByOwnerId;
          if (!ownerId) {
            return res.status(400).json(createErrorResponse(
              new Error('Owner ID is required for school creation'),
              400
            ));
          }

          if (req.schoolOwnerId && BigInt(ownerId) !== BigInt(req.schoolOwnerId)) {
            return res.status(403).json(createErrorResponse(
              new Error('You are not authorized to create schools for this owner'),
              403
            ));
          }

          const currentCount = await this.service.countActiveSchoolsForOwner(ownerId);
          if (Number(currentCount) >= Number(limits.maxSchools)) {
            return res.status(403).json(createErrorResponse(
              new Error('You have reached the maximum number of schools allowed by your subscription'),
              403,
              { currentCount, maxSchools: limits.maxSchools }
            ));
          }
        }
      }
      ...
      if (!user?.isSuperDuperAdmin && user?.schoolId) {
        try {
          await updateSubscriptionUsage(user.schoolId);
        } catch (usageError) {
          console.warn('Failed to update subscription usage after school creation:', usageError);
        }
      }
  ...
  async bulkCreateSchools(req, res) {
    try {
      ...
      // Subscription limit checks for bulk operation (non platform-admin users)
      if (!req.user?.isSuperDuperAdmin) {
        const limits = req.subscriptionLimits || {};
        if (limits?.maxSchools !== null && limits?.maxSchools !== undefined) {
          const primaryOwnerId = schools[0]?.ownerId;
          const ownerId = req.body.ownerId || primaryOwnerId || req.schoolOwnerId || req.user?.createdByOwnerId;
          if (!ownerId) {
            return res.status(400).json(createErrorResponse(
              new Error('Owner ID is required for bulk school creation'),
              400
            ));
          }
          ...
          const inconsistentOwner = schools.some((school) => school.ownerId && BigInt(school.ownerId) !== BigInt(ownerId));
          if (inconsistentOwner) {
            return res.status(400).json(createErrorResponse(
              new Error('All schools in the bulk request must belong to the same owner'),
              400
            ));
          }

          const currentCount = await this.service.countActiveSchoolsForOwner(ownerId);
          if (Number(currentCount) + schools.length > Number(limits.maxSchools)) {
            return res.status(403).json(createErrorResponse(
              new Error('Bulk operation exceeds the maximum allowed schools for your subscription'),
              403,
              {
                currentCount,
      ...
      if (!req.user?.isSuperDuperAdmin && req.user?.schoolId) {
        try {
          await updateSubscriptionUsage(req.user.schoolId);
        } catch (usageError) {
          console.warn('Failed to update subscription usage after bulk school creation:', usageError);
        }
      }
