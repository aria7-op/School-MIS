  ...
  export const updateClass = async (req, res) => {
    try {
      ...
      // Ensure we don't pass any datetime fields that could cause issues
      // Remove any potential datetime fields from the data object
  ...
  export const getClassesByTeacher = async (req, res) => {
    try {
      ...
      const where = {
        OR: [
          { classTeacherId: BigInt(teacherId) }, // Teacher is the class teacher
          { 
            teachers: { 
              some: { 
      ...
      // Add other filters
      if (params.schoolId) where.schoolId = BigInt(params.schoolId);
      if (params.level) where.level = Number(params.level);
      if (params.section) where.section = params.section;
      if (params.search) {
        ...
        where.AND = [
          { OR: where.OR }, // Keep the teacher condition
          { OR: searchConditions } // Add search condition
        ];
        delete where.OR; // Remove the original OR
      }
  ...
  export const getClassSubjects = async (req, res) => {
    try {
      ...
      teacherClassSubjects.forEach(tcs => {
        if (!subjectsMap.has(tcs.subject.id.toString())) {
          subjectsMap.set(tcs.subject.id.toString(), {
            ...tcs.subject,
            teachers: []
          });
        }
        // Add teacher to this subject
        subjectsMap.get(tcs.subject.id.toString()).teachers.push(tcs.teacher);
      });
  ...
  export const getClassesByCurrentTeacher = async (req, res) => {
    try {
      ...
      return handleError(error, res, 'get classes by current teacher');
    }
  };
  ...
  export const addSubjectToClass = async (req, res) => {
    try {
      ...
      // Verify class and subject exist in same school if school scoped
      ...
      return handleError(error, res, 'add subject to class');
    }
  };
  ...
  export const getClassTeachers = async (req, res) => {
    try {
      ...
      return handleError(error, res, 'get class teachers');
    }
  };
  ...
  export const removeTeacherFromClass = async (req, res) => {
    try {
      ...
      return handleError(error, res, 'remove teacher from class');
    }
  };
