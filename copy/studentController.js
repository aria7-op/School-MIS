...
const fetchScopedStudentIds = async (scope) => {
  ...
  if (scope.courseId !== null && scope.courseId !== undefined) {
    ...
    filters.push('se.`status` IN (?, ?)'); // Only show active enrollments
...
class StudentController {
  ...
  async assignCourse(req, res) {
    try {
      ...
      if (!school || !school.academicSessionId) {
        return createErrorResponse(res, 400, 'No current academic session found for school');
      }

      // Create new enrollment (allow multiple enrollments per student)
      const enrollment = await prisma.studentEnrollment.create({
        data: {
          studentId: parseInt(id),
          classId: parseInt(courseId), // Using courseId as classId for course enrollment
          academicSessionId: school.academicSessionId,
          courseId: parseInt(courseId),
          schoolId: req.user.schoolId,
          enrollmentDate: new Date(),
          status: 'ENROLLED', // Changed from ACTIVE to ENROLLED to match schema
          createdBy: req.user.id
        }
      });
