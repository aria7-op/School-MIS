...
const ENCRYPTION_KEY = process.env.REACT_APP_API_ENCRYPTION_KEY || 'c95fe0b21339143a5e9bda7fd12415e8';
...
/**
 * Decrypt customer data from API response
 */
export const decryptCustomerData = (encryptedResponse: any): Customer[] => {
  try {
    if (!encryptedResponse || !encryptedResponse.encryptedData) {
      return [];
    }

    // Decrypt the data
    const bytes = CryptoJS.AES.decrypt(encryptedResponse.encryptedData, ENCRYPTION_KEY);
    ...
    if (Array.isArray(decryptedData)) {
      customers = decryptedData;
    } else if (decryptedData.data && Array.isArray(decryptedData.data)) {
      customers = decryptedData.data;
    } else if (decryptedData.customers && Array.isArray(decryptedData.customers)) {
      customers = decryptedData.customers;
    } else {
      return [];
    }
