...
const useCustomerApi = () => {
  ...
  const getUnconvertedCustomers = useCallback(async (page = 1, limit = 10) => {
    ...
    try {
      ...
      return {
        customers,
        total: meta.total || customers.length || 0,
        page: meta.page || page,
  ...
  const getConversionHistory = useCallback(async (page = 1, limit = 10) => {
    ...
    try {
      ...
      const conversions = Array.isArray(response.data) ? response.data : response.data?.conversions || [];
      const meta = response.meta || response.pagination || {};

      return {
        conversions,
        total: meta.total || conversions.length || 0,
        page: meta.page || page,
        limit: meta.limit || limit,
        totalPages: meta.pages || meta.totalPages || 1,
      };
  ...
  const getConvertedStudents = useCallback(async (page = 1, limit = 10) => {
    ...
    setError(null);
    try {
      const response = await secureApiService.get(`/customers/converted?page=${page}&limit=${limit}`);
      const students = Array.isArray(response.data) ? response.data : response.data?.students || [];
      const meta = response.meta || response.pagination || {};

      return {
        students,
        total: meta.total || students.length || 0,
        page: meta.page || page,
        limit: meta.limit || limit,
        totalPages: meta.pages || meta.totalPages || 1,
      };
