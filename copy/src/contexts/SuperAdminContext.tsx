...
export interface SuperAdminContextValue {
  ...
  currentSchoolDetail: PlatformSchoolDetail | null;
  schoolDetailQuery: UseQueryResult<PlatformSchoolDetail, unknown>;
  ...
  churnAnalytics: PlatformChurnAnalytics | null;
  churnAnalyticsQuery: UseQueryResult<PlatformChurnAnalytics, unknown>;
  auditLogs: PlatformAuditLogEntry[];
  auditLogsResult: PaginatedResult<PlatformAuditLogEntry> | null;
  auditLogQuery: UseQueryResult<PaginatedResult<PlatformAuditLogEntry>, unknown>;
...
export const SuperAdminProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const churnAnalyticsQuery = useQuery({
    queryKey: platformQueryKeys.churnAnalytics('30d'),
    queryFn: () => platformService.getChurnAnalytics('30d'),
    enabled: isSuperDuperAdmin,
    staleTime: 60 * 1000,
  });
  ...
  const auditLogQuery = useQuery({
    queryKey: platformQueryKeys.auditLogs(),
  ...
  const refreshPlatform = useCallback(async () => {
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: platformQueryKeys.churnAnalytics('30d') }),
      queryClient.invalidateQueries({ queryKey: ['platform', 'analytics', 'revenue'] }),
      queryClient.invalidateQueries({ queryKey: ['platform', 'analytics', 'growth'] }),
      queryClient.invalidateQueries({ queryKey: ['platform', 'analytics', 'packages'] }),
      queryClient.invalidateQueries({ queryKey: ['platform', 'analytics', 'attendance'] }),
      ...
      selectedSchoolId
        ...
        : Promise.resolve(),
      queryClient.invalidateQueries({ queryKey: platformQueryKeys.auditLogs() }),
    ]);
  }, [queryClient, selectedSchoolId]);

  const auditLogsResult = auditLogQuery.data ?? null;
  const auditLogs = auditLogsResult?.data ?? [];
  const churnAnalytics = churnAnalyticsQuery.data ?? null;
  ...
  const contextValue: SuperAdminContextValue = useMemo(
    () => ({
      ...
      selectSchool,
      currentSchoolDetail: schoolDetailQuery.data ?? null,
      schoolDetailQuery,
