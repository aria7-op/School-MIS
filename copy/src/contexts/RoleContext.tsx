...
const getIconForResource = (resourceType: string): string => {
  const iconMap: { [key: string]: string } = {
    ...
    'COURSE_MANAGER': 'book-open-variant'
  };
  return iconMap[resourceType] || 'cog';
};

const extendRolesWithSuperDuper = <T extends { roles: string[] }>(items: T[]): T[] =>
  items.map((item) => ({
    ...item,
    roles: Array.from(new Set([...item.roles, 'SUPER_DUPER_ADMIN'])),
  }));

// Default features based on permissions
const getDefaultFeatures = (): Feature[] => extendRolesWithSuperDuper([
  {
    id: 'dashboard',
    name: 'Dashboard',
...
// Default components
const getDefaultComponents = (): Component[] => extendRolesWithSuperDuper([
  {
    id: 'student-list',
    name: 'Student List',
...
export const RoleProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getFallbackPermissions = (role: string): string[] => {
    ...
    const superAdminBasePermissions: string[] = [
      // Dashboard
      'dashboard:view',
    ...
    const ownerPermissions = [...superAdminBasePermissions];

    const rolePermissions: { [key: string]: string[] } = {
      SUPER_DUPER_ADMIN: [
        ...new Set([
          ...superAdminBasePermissions,
          'platform:schools:read',
          'platform:schools:create',
          'platform:schools:update',
          'platform:schools:delete',
          'platform:packages:read',
          'platform:packages:create',
      ...
      SUPER_ADMIN: superAdminBasePermissions,
    ...
    return (
      rolePermissions[normalizedRoleName] ??
