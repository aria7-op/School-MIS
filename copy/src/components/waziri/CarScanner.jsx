...
/**
 * Car Scanner Component
 * Main component for Waziri users to scan cars with camera controls
 */
const CarScanner = () => {
   ...
   * Process car entry
   */
  const processCarEntry = () => {
    if (scannedData) {
      // Process car entry logic
      console.log('Processing car entry:', scannedData);
      setScannerState(SCANNER_STATES.IDLE);
      setScannedData(null);
    }
  };
   ...
   * Process car exit
   */
  const processCarExit = () => {
    if (scannedData) {
      // Process car exit logic
      console.log('Processing car exit:', scannedData);
      setScannerState(SCANNER_STATES.IDLE);
      setScannedData(null);
    }
  };
    ...
    <div className="space-y-6" dir="rtl">
      ...
      {scannedData && (
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="flex space-x-4">
            <button
              onClick={processCarEntry}
              className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded text-sm font-medium transition-colors"
            >
              پردازش ورود
            </button>
            <button
              onClick={processCarExit}
