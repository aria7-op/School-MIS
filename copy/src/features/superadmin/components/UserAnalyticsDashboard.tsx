...
import { FaUsers, FaUserGraduate, FaChalkboardTeacher, FaUserTie, FaUserFriends } from 'react-icons/fa';
...
const UserAnalyticsDashboard: React.FC<Props> = ({ dateRange, selectedSchoolId, selectedBranchId, selectedCourseId }) => {
  ...
  const { data: usersOverview, isLoading, refetch } = useQuery({
    queryKey: ['users-overview', selectedSchoolId ?? null, selectedBranchId ?? null, selectedCourseId ?? null],
    queryFn: () => superadminService.getUsersOverview({ schoolId: selectedSchoolId || undefined, branchId: selectedBranchId || undefined, courseId: selectedCourseId || undefined })
  });
  ...
  const overviewFromFiltered = {
    ...
    activeRate: filteredUsers.length > 0 ? Math.round((filteredUsers.filter(isActive).length / filteredUsers.length) * 100) : 0,
  };

  // Helpers to compute teacher counts from filtered list
  const norm = (v: any): string => String(v || '').replace(/-/g, '_').toUpperCase();
  const hasRole = (u: any, role: string): boolean => {
    ...
    return rolesArr.some((r: any) => norm(r) === target || norm(r?.name) === target);
  };
  const isTeacher = (u: any) => hasRole(u, 'TEACHER');
  const filteredTeacherCount = filteredUsers.filter(isTeacher).length;
  const filteredActiveTeacherCount = filteredUsers.filter((u: any) => isTeacher(u) && (u.isActive === true || u.active === true)).length;

  const { data: studentAnalytics } = useQuery({
    queryKey: ['student-analytics', dateRange, selectedSchoolId ?? null, selectedBranchId ?? null],
    queryFn: () => superadminService.getStudentAnalytics({ ...dateRange, schoolId: selectedSchoolId || undefined, branchId: selectedBranchId || undefined })
  });

  const { data: teacherAnalytics } = useQuery({
    queryKey: ['teacher-analytics', selectedSchoolId ?? null, selectedBranchId ?? null, selectedCourseId ?? null],
    queryFn: () => superadminService.getTeacherAnalytics({ schoolId: selectedSchoolId || undefined, branchId: selectedBranchId || undefined, courseId: selectedCourseId || undefined })
  });
  ...
  const rawUsers = usersOverview?.data || usersOverview;
  const overview = rawUsers?.overview;
  const byRoleRaw = rawUsers?.byRole || [];
  ...
  const byRole = (usersList ? (() => {
    ...
    filteredUsers.forEach(u => {
      const rolesArr = Array.isArray(u.roles) && u.roles.length > 0 ? u.roles : [u.role];
    ...
    return Object.entries(map).map(([role, count]) => ({ role, count, percentage: total ? Math.round((count / total) * 100) : 0 }));
  ...
  const studentData = studentAnalytics?.data || studentAnalytics;
  const teacherData = teacherAnalytics?.data || teacherAnalytics;
  ...
  return (
    <div className="space-y-1 sm:space-y-6">
      ...
      {activeTab === 'users' && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-lg font-semibold text-gray-900">{t('superadmin.users.usersList', 'Users List')}</h3>
            {canCreateUsers && (
              <button
                type="button"
          ...
          {loadingUsers ? (
            ...
            <div className="overflow-x-auto">
                    ...
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${
                          ...
                          {user.isActive ? t('superadmin.users.table.active', 'Active') : t('superadmin.users.table.inactive', 'Inactive')}
                        </span>
                      </td>
              ...
              {(!usersList?.data && !usersList) || (usersList?.data || usersList || []).length === 0 && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-1 sm:gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-600">{t('superadmin.users.totalUsers', 'Total Users')}</span>
            <FaUsers className="w-5 h-5 text-blue-600" />
          </div>
          <p className="text-3xl font-bold text-gray-900">{(overviewFromFiltered.totalUsers || overview?.totalUsers || 0).toLocaleString()}</p>
          <p className="text-xs text-gray-500 mt-2">{t('superadmin.users.activeRate', { percent: overviewFromFiltered.activeRate || overview?.activeRate || 0 }) || `${overviewFromFiltered.activeRate || overview?.activeRate || 0}% active`}</p>
        </div>

        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-600">{t('superadmin.users.totalStudents', 'Total Students')}</span>
            <FaUserGraduate className="w-5 h-5 text-green-600" />
          </div>
          <p className="text-3xl font-bold text-gray-900">{studentData?.total?.toLocaleString() || 0}</p>
          <p className="text-xs text-gray-500 mt-2">
            {studentData?.demographics?.malePercentage || 0}% {t('superadmin.users.male', 'M')} / {studentData?.demographics?.femalePercentage || 0}% {t('superadmin.users.female', 'F')}
          </p>
        </div>

        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-600">{t('superadmin.users.totalTeachers', 'Total Teachers')}</span>
            <FaChalkboardTeacher className="w-5 h-5 text-purple-600" />
          </div>
          <p className="text-3xl font-bold text-gray-900">{teacherData?.total?.toLocaleString() || 0}</p>
          <p className="text-xs text-gray-500 mt-2">{t('superadmin.users.activeTeachers', { count: teacherData?.active || 0 }) || `${teacherData?.active || 0} active`}</p>
        </div>

        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-gray-600">{t('superadmin.users.activeUsers', 'Active Users')}</span>
            <FaUserTie className="w-5 h-5 text-orange-600" />
          </div>
          <p className="text-3xl font-bold text-orange-600">{overview?.activeUsers?.toLocaleString() || 0}</p>
          <p className="text-xs text-gray-500 mt-2">{t('superadmin.users.inactiveUsers', { count: overview?.inactiveUsers || 0 }) || `${overview?.inactiveUsers || 0} inactive`}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('superadmin.users.usersByRole', 'Users by Role')}</h3>
            ...
            <PieChart>
              ...
              <Tooltip 
                ...
                formatter={(value: any, _name: any, props: any) => {
                  ...
                  return [`${value} (${percent}%)`, roleLabel(props?.payload?.role)];
                }} 
              />
              <Legend 
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('superadmin.users.roleDistribution', 'Role Distribution')}</h3>
          <div className="space-y-4">
            {byRole.map((role, index) => (
              <div key={index} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50">
                <div>
                  <p className="text-sm font-medium text-gray-900">{roleLabel(role.role)}</p>
                  <p className="text-xs text-gray-500">{t('superadmin.users.ofTotalUsers', { percent: role.percentage }) || `${role.percentage}% of total users`}</p>
                </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('superadmin.users.studentDemographics', 'Student Demographics')}</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center p-6 bg-blue-50 rounded-lg">
            <FaUserGraduate className="w-12 h-12 text-blue-600 mx-auto mb-3" />
            <p className="text-sm font-medium text-gray-600 mb-2">{t('superadmin.users.totalStudents', 'Total Students')}</p>
            <p className="text-3xl font-bold text-blue-600">{studentData?.total?.toLocaleString() || 0}</p>
          </div>

          <div className="text-center p-6 bg-purple-50 rounded-lg">
            <FaUserFriends className="w-12 h-12 text-purple-600 mx-auto mb-3" />
            <p className="text-sm font-medium text-gray-600 mb-2">{t('superadmin.users.maleStudents', 'Male Students')}</p>
            <p className="text-3xl font-bold text-purple-600">{studentData?.demographics?.male?.toLocaleString() || 0}</p>
            <p className="text-xs text-gray-500 mt-1">{studentData?.demographics?.malePercentage || 0}%</p>
          </div>

          <div className="text-center p-6 bg-pink-50 rounded-lg">
            <FaUserFriends className="w-12 h-12 text-pink-600 mx-auto mb-3" />
            <p className="text-sm font-medium text-gray-600 mb-2">{t('superadmin.users.femaleStudents', 'Female Students')}</p>
            <p className="text-3xl font-bold text-pink-600">{studentData?.demographics?.female?.toLocaleString() || 0}</p>
            <p className="text-xs text-gray-500 mt-1">{studentData?.demographics?.femalePercentage || 0}%</p>
          </div>
        </div>
      </div>
      ...
      {/* Teacher Analytics */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('superadmin.users.teacherAnalytics', 'Teacher Analytics')}</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="text-center p-6 border border-gray-200 rounded-lg">
            <p className="text-sm font-medium text-gray-600 mb-2">{t('superadmin.users.totalTeachers', 'Total Teachers')}</p>
            <p className="text-3xl font-bold text-gray-900">{(filteredTeacherCount || teacherData?.total || 0).toLocaleString()}</p>
          </div>

          <div className="text-center p-6 border border-gray-200 rounded-lg">
            <p className="text-sm font-medium text-gray-600 mb-2">{t('superadmin.users.activeTeachersLabel', 'Active Teachers')}</p>
            <p className="text-3xl font-bold text-green-600">{(filteredActiveTeacherCount || teacherData?.active || 0).toLocaleString()}</p>
          </div>

          <div className="text-center p-6 border border-gray-200 rounded-lg">
            <p className="text-sm font-medium text-gray-600 mb-2">{t('superadmin.users.avgClassLoad', 'Avg Class Load')}</p>
            <p className="text-3xl font-bold text-blue-600">{teacherData?.workload?.averageClasses?.toFixed(1) || 0}</p>
          </div>
        </div>
      </div>
