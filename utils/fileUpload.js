...
class FileUpload {
    ...
    /**
     * Upload a file to the specified directory with organized folder structure
     * @param {Object} file - The file object from multer or similar
     * @param {String} directory - The base directory (e.g., 'assignments', 'submissions')
     * @param {Object} options - Additional options for folder organization
     * @param {String} options.teacherId - Teacher ID for folder organization
     * @param {String} options.classId - Class ID for folder organization  
     * @param {String} options.subjectId - Subject ID for folder organization
    ...
    async uploadFile(file, directory, options = {}) {
        try {
            // Debug logging
            logger.info(`Upload file called with directory: ${directory}, options:`, options);
            ...
            if (options.teacherId) {
                dirPath = path.join(dirPath, `teacher-${options.teacherId}`);
                
                if (options.classId) {
                    dirPath = path.join(dirPath, `class-${options.classId}`);
                    
                    if (options.subjectId) {
                        dirPath = path.join(dirPath, `subject-${options.subjectId}`);
                    }
                }
            }

            logger.info(`Final directory path: ${dirPath}`);

            // Create directory structure if it doesn't exist
            const uploadDir = path.join(process.cwd(), 'uploads', dirPath);
            if (!fs.existsSync(uploadDir)) {
                fs.mkdirSync(uploadDir, { recursive: true });
                logger.info(`Created directory structure: ${dirPath}`);
            }

            // Generate unique filename
            const fileExt = path.extname(file.originalname) || mime.extension(file.mimetype) || '.bin';
            const filename = `files-${Date.now()}-${Math.random().toString(36).substring(2)}${fileExt}`;
            const filePath = path.join(uploadDir, filename);
            const relativePath = path.join('uploads', dirPath, filename).replace(/\\/g, '/');
