...
class AuditLogger {
    ...
    static async createAuditLog({
        ...
        try {
            ...
            if (details) structuredData.details = details;
            if (metadata) structuredData.metadata = metadata;

            const payload = {
                action,
                entityType: resource,
                entityId: resourceId !== null ? BigInt(resourceId) : BigInt(0),
                oldData: null,
                newData: Object.keys(structuredData).length > 0 ? JSON.stringify(structuredData) : null,
                ...
                requestQuery,
                requestBody,
                responseStatus: responseStatus !== null ? parseInt(responseStatus) : null,
                responseTimeMs: responseTimeMs !== null ? parseInt(responseTimeMs) : null,
                isSuccess: typeof isSuccess === 'boolean' ? isSuccess : Boolean(isSuccess),
                errorMessage,
                correlationId,
                traceId
            };
