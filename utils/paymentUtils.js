...
const paymentSchema = Joi.object({
  ...
  paymentMonth: Joi.string().optional(),
...
export const createPaymentLog = async (paymentId, action, oldValue, newValue, ipAddress, userAgent, schoolId, userId) => {
  try {
    // Check if paymentLog model exists in Prisma client
    if (!prisma.paymentLog) {
      console.log('PaymentLog model not available in Prisma client - skipping log creation');
      return;
    }
    ...
    // Don't throw - payment log is not critical
  }
};
