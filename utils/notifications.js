...
export const buildNotificationIncludeQuery = (include) => {
  ...
  if (includes.includes('deliveryStatus') || includes.includes('deliveries')) {
    includeObj.deliveries = {
      select: {
        id: true,
        channel: true,
        status: true,
  ...
  if (!minimal) {
    ...
    if (notification.recipients) {
      response.recipients = notification.recipients.map(recipient => ({
        ...
        notificationId: recipient.notificationId?.toString(),
    ...
    if (notification.school) {
      response.school = {
        id: notification.school.id?.toString(),
    ...
    if (notification.deliveries) {
      response.deliveryStatus = notification.deliveries.map(status => ({
 ...
 */
export const buildNotificationSearchQuery = async (filters, userId, schoolId, userRole) => {
  // For SUPER_ADMIN, show ALL notifications in their school (they see everything!)
  if (userRole === 'SUPER_ADMIN') {
    ...
    console.log('✅ SUPER_ADMIN: Showing ALL school notifications');
    return where;
  }

  // For regular users, only show notifications where they're recipients or senders
  const where = {
    schoolId,
    OR: [
  ...
  notifications.forEach(notification => {
    ...
    notification.recipients.forEach(recipient => {
      ...
      if (recipient.channel) {
        stats.deliveryChannels[recipient.channel]++;
      }
  ...
  try {
    ...
    const notification = await prisma.notification.create({
      data: {
        ...completeData,
        recipients: {
          create: completeData.recipients.map(userId => ({
            userId,
      ...
      include: buildNotificationIncludeQuery('deliveries,attachments')
    });
      ...
      updates.push(
        prisma.notificationDelivery.updateMany({
          ...
          data: {
            ...
            errorMessage: status === 'FAILED' ? result.reason.message : null
          }
        })
      );
    });
