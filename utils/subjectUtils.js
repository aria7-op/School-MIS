...
export const SubjectCreateSchema = z.object({
  ...
  code: z.coerce.string().min(1, 'Subject code is required').max(20, 'Subject code must be less than 20 characters'),
  ...
  weeklyHoursPerClass: z.record(z.string(), z.number().int().min(0).max(40)).optional()
});

export const SubjectUpdateSchema = z.object({
  name: z.string().min(1, 'Subject name is required').max(100, 'Subject name must be less than 100 characters').optional(),
  code: z.coerce.string().min(1, 'Subject code is required').max(20, 'Subject code must be less than 20 characters').optional(),
  description: z.string().max(1000, 'Description must be less than 1000 characters').optional(),
 ...
 */
export const validateSubjectConstraints = async (schoolId, code, departmentId = null, excludeId = null) => {
  try {
    const errors = [];

    console.log('🔍 Validating subject constraints:', {
      schoolId,
      code,
      departmentId,
      excludeId
    ...
    // Note: Unique code validation removed to allow duplicate subject codes
    ...
    // Check if department exists and belongs to school (only if departmentId is provided and not null)
    if (departmentId && departmentId !== 'null' && departmentId !== null) {
      ...
      if (!department) {
        // Check if any departments exist for this school
        const anyDepartments = await prisma.department.findMany({
          where: {
            schoolId: BigInt(schoolId),
            deletedAt: null
          },
        ...
        console.log('📋 Available departments in school:', anyDepartments);
        
        // Only add error if departments exist but the specified one is not found
        if (anyDepartments.length > 0) {
          errors.push(`Department with ID ${departmentId} does not exist or does not belong to the school. Available departments: ${anyDepartments.map(d => `${d.id}:${d.name}`).join(', ')}`);
        } else {
          console.log('ℹ️ No departments in school, skipping department validation');
        }
      }
    }
    ...
    return {
      isValid: false,
      errors: ['Failed to validate subject constraints: ' + error.message]
    };
  }
};
 ...
 */
export const buildSubjectSearchQuery = (filters, schoolId, userRole = null) => {
  const where = {
    schoolId: BigInt(schoolId),
    deletedAt: null
  };
  ...
  // This allows inactive subjects to be included in the results
  if (filters.isActive !== undefined && filters.isActive !== null) {
    // Convert string 'true'/'false' to boolean if needed
    const isActiveValue = filters.isActive === 'true' || filters.isActive === true || filters.isActive === 1 || filters.isActive === '1';
    where.isActive = isActiveValue;
  }
  // If no isActive filter is provided, include both active and inactive subjects
