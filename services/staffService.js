...
const AFGHANISTAN_UTC_OFFSET_MIN = 270;
...
const getAfghanistanDayRangeUTC = (input) => {
  ...
  const startUTCms =
    Date.UTC(year, month, day, 0, 0, 0, 0) - AFGHANISTAN_UTC_OFFSET_MIN * 60 * 1000;
  const endUTCms =
    Date.UTC(year, month, day, 23, 59, 59, 999) - AFGHANISTAN_UTC_OFFSET_MIN * 60 * 1000;
  return { startOfDayUTC: new Date(startUTCms), endOfDayUTC: new Date(endUTCms) };
};

const ensureStaffScope = (scope, contextLabel = 'staff operation') => {
  const normalized = {
...
const resolveScopeInput = (primary, contextLabel) => {
  if (primary && typeof primary === 'object' && primary !== null && 'schoolId' in primary) {
    return ensureStaffScope(primary, contextLabel);
  }

  return ensureStaffScope({ schoolId: primary }, contextLabel);
};

const buildScopedStaffWhere = (scope, baseWhere = {}, { useBranch = true, useCourse = false } = {}) => {
...
const coerceStaffId = (staffId, contextLabel = 'staff identifier') => {
  const id = toBigIntSafe(staffId, null);
  if (id === null) {
    const error = new Error(`Invalid ${contextLabel}`);
    error.statusCode = 400;
    throw error;
  }
  return id;
...
const ensureStaffAccessible = async (staffId, scope, include) => {
  const staffIdBigInt = coerceStaffId(staffId);
  ...
  if (!staff) {
    const error = new Error('Staff not found in managed scope');
...
const buildScopeCacheKey = (scope) => ({
...
class StaffService {
  ...
  async getTodaysStaffLeaveSet(staffMembers, scope) {
    ...
    // Get teacher IDs for users who have teacher records
    let teacherIds = [];
    const teacherIdToUserId = new Map();
    if (userIds.length > 0) {
      ...
      teachers.forEach((t) => {
        teacherIdToUserId.set(t.id.toString(), t.userId.toString());
      });
    }
    ...
    if (staffIds.length > 0) {
      whereClause.OR.push({ staffId: { in: staffIds } });
    }
    if (teacherIds.length > 0) {
      whereClause.OR.push({ teacherId: { in: teacherIds } });
    }
    ...
    const leaveRecords = await this.prisma.attendance.findMany({
      where: whereClause,
      select: { staffId: true, teacherId: true },
    });

    // Build a map of userId -> isOnLeave
    const userIdToLeaveStatus = new Map();
    
    // Map staffId -> userId
    const staffIdToUserId = new Map();
    staffMembers.forEach((member) => {
      if (member?.id && member?.userId) {
        staffIdToUserId.set(member.id.toString(), member.userId.toString());
      }
    });

    // Mark users as on leave
    leaveRecords.forEach((record) => {
      if (record.staffId) {
        const userId = staffIdToUserId.get(record.staffId.toString());
        if (userId) userIdToLeaveStatus.set(userId, true);
      }
      if (record.teacherId) {
        const userId = teacherIdToUserId.get(record.teacherId.toString());
        if (userId) userIdToLeaveStatus.set(userId, true);
      }
    });

    // Return a set of staff IDs that are on leave
    const onLeaveStaffIds = new Set();
    staffMembers.forEach((member) => {
      if (member?.userId && userIdToLeaveStatus.has(member.userId.toString())) {
        if (member?.id) {
          onLeaveStaffIds.add(member.id.toString());
        }
      }
    });

    return onLeaveStaffIds;
  }
  ...
  async createStaff(data, userId, scopeInput) {
    try {
        ...
        try {
          ...
          if (maxStaff !== null && maxStaff !== undefined) {
            ...
            if (Number(currentCount) + 1 > Number(maxStaff)) {
              const err = new Error('Staff limit reached for current subscription');
              err.code = 'LIMIT_EXCEEDED';
              err.meta = { schoolId, currentCount, maxStaff };
              throw err;
            }
          }
        } catch (limitError) {
          logger.warn('Failed to enforce staff package limit:', limitError);
        }
      }

      const createdById = toBigIntSafe(userId, null);
      if (createdById === null) {
        const error = new Error('Invalid user identifier for staff creation');
        error.statusCode = 400;
        throw error;
      }

      const branchId = enforceBranchAgainstScope(data.branchId, scope);
      const departmentId = toBigIntOrNull(data.departmentId);
      ...
      try {
        ...
        logger.warn('Failed to update subscription usage after staff creation:', usageError);
      }
  ...
  async getStaff(filters, scopeInput, include = null) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff listing');
      ...
      const searchQuery = buildStaffSearchQuery(filters || {});
      const includeQuery = buildStaffIncludeQuery(include);

      if (searchQuery.departmentId !== undefined && searchQuery.departmentId !== null) {
        const deptId = toBigIntOrNull(searchQuery.departmentId);
        searchQuery.departmentId = deptId ?? undefined;
      }

      const where = buildScopedStaffWhere(scope, searchQuery);
      ...
      const leaveSet = await this.getTodaysStaffLeaveSet(staff, scope);

      const formattedStaff = staff.map((member) => {
        const formatted = formatStaffResponse(member, true);
        const isOnLeave = leaveSet.has(member.id.toString());
        formatted.leaveStatus = isOnLeave ? 'ON_LEAVE' : 'AVAILABLE';
        formatted.isOnLeave = isOnLeave;
        if (formatted.user) {
          formatted.user.leaveStatus = formatted.leaveStatus;
          formatted.user.isOnLeave = isOnLeave;
        }
        return formatted;
      });
  ...
  async getStaffById(staffId, scopeInput, include = null) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff details');
      const cacheKey = `byId:${staffId}:${JSON.stringify(buildScopeCacheKey(scope))}:${include}`;
      // const cached = await this.getFromCache(cacheKey);
      // if (cached) return cached;

      const includeObj = buildStaffIncludeQuery(include);
      const { staff } = await ensureStaffAccessible(staffId, scope, includeObj);

      const leaveSet = await this.getTodaysStaffLeaveSet([staff], scope);
      const result = formatStaffResponse(staff, true);
      const isOnLeave = leaveSet.has(staff.id.toString());
      result.leaveStatus = isOnLeave ? 'ON_LEAVE' : 'AVAILABLE';
      result.isOnLeave = isOnLeave;
      if (result.user) {
        result.user.leaveStatus = result.leaveStatus;
        result.user.isOnLeave = isOnLeave;
      }
  ...
  async updateStaff(staffId, data, userId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff update');
      const { staff: existingStaff, staffIdBigInt } = await ensureStaffAccessible(
        staffId,
        scope,
        { user: true }
      );
      ...
      if (updatedById === null) {
        const error = new Error('Invalid user identifier for staff update');
        error.statusCode = 400;
        throw error;
      }

      // Check if staff exists and user has permission
      const hasPermission = await validateStaffPermissions(
        staffIdBigInt,
        updatedById,
        scope.schoolId
      );
      ...
      // Validate data
      const validationErrors = await validateStaffData(data, scope.schoolId, existingStaff.userId);
      if (validationErrors.length > 0) {
        throw new Error(`Validation errors: ${validationErrors.join(', ')}`);
      }

      const nextBranchId = data.branchId !== undefined
        ? enforceBranchAgainstScope(data.branchId, scope)
        : existingStaff.branchId;
      const nextDepartmentId = data.departmentId !== undefined
        ? toBigIntOrNull(data.departmentId)
      ...
      const result = await this.prisma.$transaction(async (tx) => {
        ...
        if (Object.keys(data).some(key => ['email', 'phone', 'firstName', 'middleName', 'lastName', 'displayName', 'gender', 'birthDate', 'avatar', 'bio', 'timezone', 'locale', 'metadata'].includes(key))) {
          ...
          userData.updatedBy = updatedById;
        ...
        if (data.departmentId !== undefined) staffData.departmentId = nextDepartmentId;
        ...
        staffData.branchId = nextBranchId;
        staffData.updatedBy = updatedById;

        const staff = await tx.staff.update({
          where: { id: staffIdBigInt },
          data: staffData,
          include: {
            user: true,
            department: true,
      ...
      // Invalidate cache
      await this.invalidateStaffCache(staffIdBigInt, scope.schoolId);

      // Create audit log
      await createAuditLog({
        action: 'UPDATE',
        entityType: 'Staff',
        entityId: staffIdBigInt,
        userId: updatedById,
        schoolId: scope.schoolId,
        oldData: null,
        newData: {
          staffId: staffIdBigInt,
          employeeId: result.employeeId,
          email: result.user.email,
          name: `${result.user.firstName} ${result.user.lastName}`,
          changes: data
      ...
      logger.info(`Staff updated: ${staffIdBigInt} by user: ${userId}`);
      return formatStaffResponse(result, true);
  ...
  async deleteStaff(staffId, userId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff deletion');
      const { staff, staffIdBigInt } = await ensureStaffAccessible(
        staffId,
        scope,
        { user: true }
      );
      ...
      const updatedById = toBigIntSafe(userId, null) ?? userId;
      ...
      logger.info(`Staff deleted: ${staffIdBigInt} by user: ${userId}`);
  ...
  async restoreStaff(staffId, userId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff restore');
      const { staff, staffIdBigInt } = await ensureStaffAccessible(
        staffId,
        scope,
        { user: true }
      ...
      logger.info(`Staff restored: ${staffIdBigInt} by user: ${userId}`);
  ...
  async bulkCreateStaff(data, userId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'bulk staff creation');
      const results = [];
      const errors = [];

      for (const staffData of data.staff) {
        try {
          const result = await this.createStaff(staffData, userId, scope);
          results.push(result);
        } catch (error) {
          errors.push({
            data: staffData,
  ...
  async bulkUpdateStaff(data, userId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'bulk staff update');
      const results = [];
      const errors = [];

      for (const update of data.updates) {
        try {
          const result = await this.updateStaff(update.id, update.data, userId, scope);
          results.push(result);
        } catch (error) {
          errors.push({
            id: update.id,
  ...
  async bulkDeleteStaff(data, userId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'bulk staff delete');
      const results = [];
      const errors = [];

      for (const staffId of data.staffIds) {
        try {
          const result = await this.deleteStaff(staffId, userId, scope);
  ...
  async getStaffStats(staffId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff stats');
      const staffIdBigInt = coerceStaffId(staffId, 'staff identifier');
      const cacheKey = `stats:${staffIdBigInt}:${JSON.stringify(buildScopeCacheKey(scope))}`;
      const cached = await this.getFromCache(cacheKey);
      if (cached) return cached;

      const where = buildScopedStaffWhere(scope, { id: staffIdBigInt });
      const [staff, attendances, payrolls] = await Promise.all([
        ...
        this.prisma.attendance.count({
          where: { staffId: staffIdBigInt, ...buildScopedStaffWhere(scope, {}) }
        }),
  ...
  async getStaffAnalytics(staffId, scopeInput, period = '30d') {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff analytics');
      const staffIdBigInt = coerceStaffId(staffId, 'staff identifier');
      const cacheKey = `analytics:${staffIdBigInt}:${JSON.stringify(buildScopeCacheKey(scope))}:${period}`;
      const cached = await this.getFromCache(cacheKey);
      if (cached) return cached;
      ...
      const attendanceWhere = buildScopedStaffWhere(scope, {
        staffId: staffIdBigInt,
        date: {
          gte: startDate,
          lte: now
      ...
      const payrollWhere = buildScopedStaffWhere(scope, {
        staffId: staffIdBigInt,
        paymentDate: {
          gte: startDate,
          lte: now
  ...
  async getStaffPerformance(staffId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff performance');
      const staffIdBigInt = coerceStaffId(staffId, 'staff identifier');
      const cacheKey = `performance:${staffIdBigInt}:${JSON.stringify(buildScopeCacheKey(scope))}`;
      const cached = await this.getFromCache(cacheKey);
      if (cached) return cached;
  ...
  async searchStaff(query, scopeInput, include = null) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff search');
      const cacheKey = `search:${query}:${JSON.stringify(buildScopeCacheKey(scope))}:${include}`;
      const cached = await this.getFromCache(cacheKey);
      if (cached) return cached;
      ...
      const where = buildScopedStaffWhere(scope, baseWhere);
  ...
  async exportStaff(filters, scopeInput, format = 'json') {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff export');
      const searchQuery = buildStaffSearchQuery(filters || {});
      const where = buildScopedStaffWhere(scope, searchQuery);
  ...
  async importStaff(data, userId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff import');
      const results = [];
      const errors = [];
  ...
  async generateEmployeeIdSuggestions(designation, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'employee ID suggestions');
      const suggestions = [];
      const prefix = designation.split(' ').map(word => word.charAt(0)).join('').toUpperCase();
      
      for (let i = 1; i <= 5; i++) {
        const employeeId = `${prefix}${String(i).padStart(4, '0')}`;
        const where = buildScopedStaffWhere(scope, { employeeId });
  ...
  async getStaffCountByDepartment(scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff count by department');
      const cacheKey = `countByDepartment:${JSON.stringify(buildScopeCacheKey(scope))}`;
      const cached = await this.getFromCache(cacheKey);
      if (cached) return cached;

      const where = buildScopedStaffWhere(scope, {});
      ...
      const deptWhere = applyScopeToWhere({}, scope, { useBranch: true, useCourse: false });
  ...
  async getStaffCountByDesignation(scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff count by designation');
      const cacheKey = `countByDesignation:${JSON.stringify(buildScopeCacheKey(scope))}`;
      const cached = await this.getFromCache(cacheKey);
      if (cached) return cached;
  ...
  async getStaffBySchool(scopeInput, include = null) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff by school');
      const cacheKey = `bySchool:${JSON.stringify(buildScopeCacheKey(scope))}:${include}`;
      const cached = await this.getFromCache(cacheKey);
      if (cached) return cached;
  ...
  async warmCache(scopeInput, staffId = null) {
    try {
      const scope = resolveScopeInput(scopeInput, 'warm cache');
      if (staffId) {
        // Warm specific staff cache
        const staffIdBigInt = coerceStaffId(staffId, 'staff identifier');
        await this.getStaffById(staffIdBigInt, scope, 'department,attendances,payrolls,school');
        await this.getStaffStats(staffIdBigInt, scope);
        await this.getStaffAnalytics(staffIdBigInt, scope, '30d');
        await this.getStaffPerformance(staffIdBigInt, scope);
      } else {
        // Warm school-level cache
        await this.getStaff({ page: 1, limit: 50 }, scope, 'department,attendances,payrolls,school');
        await this.getStaffCountByDepartment(scope);
        await this.getStaffCountByDesignation(scope);
        await this.getStaffBySchool(scope, 'department,attendances,payrolls,school');
      }
  ...
  async clearCache(scopeInput = null) {
    try {
      if (scopeInput) {
        const scope = resolveScopeInput(scopeInput, 'clear cache');
        const scopeKey = JSON.stringify(buildScopeCacheKey(scope));
        await this.deleteCache(`*:${scopeKey}*`);
      } else {
  ...
  async getStaffByDepartment(departmentId, scopeInput) {
    try {
      const scope = resolveScopeInput(scopeInput, 'staff by department');
      const deptIdBigInt = toBigIntOrNull(departmentId);
      const where = buildScopedStaffWhere(scope, {
        departmentId: deptIdBigInt ?? undefined
      });
      const staff = await this.prisma.staff.findMany({
        where,
        include: {
    ...
    try {
      ...
      if (schoolId) {
        try {
          ...
          logger.warn('Failed to refresh subscription usage after staff document upload:', usageError);
        }
      }
    ...
    try {
      ...
      if (schoolId) {
        try {
          ...
          logger.warn('Failed to refresh subscription usage after staff document deletion:', usageError);
        }
      }
