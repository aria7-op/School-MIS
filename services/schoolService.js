...
class SchoolService {
  ...
  async createSchool(schoolData, createdBy) {
    try {
      ...
      const owner = await this.prisma.owner.findUnique({
        where: { id: BigInt(validatedData.ownerId) }
      });
      
      if (!owner) {
        throw new Error(`Owner with ID ${validatedData.ownerId} does not exist`);
      }
      ...
      // Transform address object to flat fields and map validation fields to database fields
      const { address: addressObj, location: locationObj, description, type, level, metadata, ...restData } = validatedData;
      const transformedData = {
        ...restData,
        about: description || null, // Map description to about field
      ...
      const school = await this.prisma.school.create({
        data: {
          ...transformedData,
          ownerId: BigInt(transformedData.ownerId),
          academicSessionId: transformedData.academicSessionId ? BigInt(transformedData.academicSessionId) : null,
          currentTermId: transformedData.currentTermId ? BigInt(transformedData.currentTermId) : null,
  ...
  async updateSchool(schoolId, updateData, updatedBy) {
    try {
      ...
      // Transform address and location objects to flat fields if present
      let transformedData = validatedData;
      if (validatedData.address && typeof validatedData.address === 'object' || 
          validatedData.location && typeof validatedData.location === 'object' ||
          validatedData.description || validatedData.type || validatedData.level ||
          validatedData.metadata) {
        const { address: addressObj, location: locationObj, description, type, level, metadata, ...restData } = validatedData;
        transformedData = {
          ...restData,
          about: description || existingSchool.about, // Map description to about field
          country: addressObj?.country || existingSchool.country,
          state: addressObj?.state || existingSchool.state,
          city: addressObj?.city || existingSchool.city,
          address: addressObj?.street || existingSchool.address,
          postalCode: addressObj?.postalCode || existingSchool.postalCode,
          latitude: locationObj?.latitude || existingSchool.latitude,
          longitude: locationObj?.longitude || existingSchool.longitude,
        };
      }
      ...
      const school = await this.prisma.school.update({
        ...
        data: {
          ...transformedData,
          academicSessionId: transformedData.academicSessionId ? BigInt(transformedData.academicSessionId) : undefined,
          currentTermId: transformedData.currentTermId ? BigInt(transformedData.currentTermId) : undefined,
  ...
  /**
   * Count active (non-deleted) schools for an owner
   */
  async countActiveSchoolsForOwner(ownerId) {
    if (!ownerId) return 0;
