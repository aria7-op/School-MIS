...
class GoogleDriveService {
  ...
  async setBillTemplate(schoolId, fileId, fileName) {
    try {
      ...
      // Note: billTemplate fields don't exist in School model
      // If bill template functionality is needed, store it in File model instead
      console.log('Bill template save skipped - Bill model not available in Prisma schema');
      // Store template in File model instead if needed
  ...
  async getBillTemplate(schoolId) {
    try {
      // Note: billTemplate fields don't exist in School model, so we skip this functionality
      // If bill template functionality is needed, it should be stored in a separate model or File model
      console.log('Bill template functionality is not available - Bill model not in schema');
      return null;
      
      // Original code commented out since fields don't exist in schema:
      /*
      const school = await prisma.school.findFirst({
        where: { id: BigInt(schoolId) },
        select: {
    ...
    try {
      await prisma.school.update({
        ...
        data: {
          ...
          // Note: billTemplate fields don't exist in School model, so they're not included
        }
      });
