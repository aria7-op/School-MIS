...
class FileGenerationService {
  ...
  async generateInvoicePDF(bill, payment, school, student = null, parent = null) {
    try {
      // If bill is null, skip invoice generation or use payment transaction ID
      if (!bill) {
        console.log('Skipping invoice generation - bill is null');
        return null;
      }
      ...
      const billNumber = bill?.billNumber || payment?.transactionId || payment?.id || 'N/A';
      const filename = `invoice-${billNumber}-${Date.now()}.pdf`;
      const filePath = path.join(this.uploadDir, filename);
  ...
  async generatePaymentFiles(payment, bill, school, student = null, parent = null) {
    try {
      ...
      // Generate invoice PDF (only if bill exists)
      if (bill) {
      ...
      files.push({
        ...invoiceFile,
            originalName: `Invoice-${billNumber}.pdf`,
        fileType: 'pdf',
            entityType: 'payment',
            description: `Invoice for payment ${payment.transactionId || payment.id}`,
